...
########ablation
    # def forward(self, x, edge, cache_name='source'):#only use knn as ablation
    #     embedding = self.encoder(x)
    #     feat_a = self.cachedgcn1(embedding, edge, cache_name)
    #     mu_a = self.cachedgcn2(feat_a, edge, cache_name)
    #     logvar_a = self.cachedgcn3(feat_a, edge, cache_name)
    #     std_a = torch.exp(logvar_a)
    #     gnn_z_a = torch.randn_like(std_a) * std_a + mu_a
    #     z = gnn_z_a

        # return z


    # def forward(self, x, edge, cache_name='source'):#only use ppmi as ablation
    #     embedding = self.encoder(x)
    #     feat_p = self.ppmi1(embedding, edge, cache_name)
    #     mu_p = self.ppmi2(feat_p, edge, cache_name)
    #     logvar_p = self.ppmi3(feat_p, edge, cache_name)
    #     std_p = torch.exp(logvar_p)
    #     gnn_z_p = torch.randn_like(std_p) * std_p + mu_p
    #     z = gnn_z_p
    #     return z
...
class STransferClassifier(nn.Module):
    def __init__(self, input_dim, p_drop=0.2):
        ...
        #     nn.Linear(32, 7)
        # )
        # self.cls_loss = nn.CrossEntropyLoss()#ablation
...
class Discriminator(nn.Module):
    ...
    # def __init__(self, input_dims=32, output_dims=2, hidden_dims=500):#ablation study
