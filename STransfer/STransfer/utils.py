...
def preprocess_slice_OV(base_path,slicename,label_mapping, device, n_top_genes=500):
    base_path = os.path.join(base_path, f"{slicename}.h5ad")
    adata = sc.read_h5ad(base_path)
    labels = adata.obs['annotation'].map(label_mapping)
    labels = torch.LongTensor(labels).to(device)
    ...
    adata.obsm['process'] = adata.obsm['X_pca']
    graph_raw = graph_construction(adata, neighbor_nodes=6)
    graph_dict = {
        "indices": torch.tensor(graph_raw["indices"], dtype=torch.long).to(device)}
    return adata,labels, graph_dict
def preprocess_slice_CRC(path,slicename,label_mapping, device, n_top_genes=3000):
    path = os.path.join(path, f"{slicename}.h5ad")
    adata = sc.read_h5ad(path)

    ann = adata.obs['Pathologist Annotation']
    valid_mask = ann.notna()

    adata = adata[valid_mask].copy()

    labels = adata.obs['Pathologist Annotation'].map(label_mapping)
    labels = torch.LongTensor(labels.values).to(device)
